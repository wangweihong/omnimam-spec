...
`application-platform` 将 Provider 能力与 ComfyUI 工作流封装为面向用户的稳定 Application。它拥有工作流、模板与应用版本、运行表单、ApplicationRun 编排和业务投影，并通过 `modelgateway` 的 `TargetSelection` 与 `ExecuteOperation` 选择并调用真实执行实现。

## 2. 核心对象

- `CapabilityDefinition`、`ProviderCapability`、`ProviderType`、`ProviderAccount`、`ProviderAccountCapabilityBinding`、`ProviderAdapter`、`OperationExecutor`：由 `modelgateway` 拥有，本领域通过受控边界消费。
- `ApplicationExecutor`：编排 ApplicationRun，以不可变执行快照构造 `TargetSelection` 并调用 Model Gateway `ExecuteOperation`。
...
- ProviderAccount 表达真实连接环境；凭证与内部配置不得进入普通摘要。
- ApplicationRun 保存不可变输入、版本、能力和执行快照，只单调投影 AtomicTask 状态。
- Application Platform 只构造非敏感 `TargetSelection` 并交由 Model Gateway 解析，不维护 Provider 专用 HTTP 客户端或直接调用 OperationExecutor。
- 无法保持应用语义兼容的工作流应创建新 Application，不能强行追加为新版本。
- Artifact 只以受控 ID 和状态摘要关联，制品事实由 asset-library 维护。

## 4. 领域边界

本领域拥有 ComfyUIWorkflow、应用、模板、版本、RuntimeFormSchema 和 ApplicationRun。能力目录、Provider、Binding、Adapter 与 OperationExecutor 归 `modelgateway`；AtomicTask 状态归 task-center；Artifact 与 Asset 归 asset-library；Canvas 图结构和编译归 workflow-canvas。

## 5. 上游与下游

上游包括 identity 的主体与权限，以及 `modelgateway` 的只读 Provider 能力、Provider 和执行入口。下游包括 task-center 的异步执行、asset-library 的制品接收、workflow-canvas 的 ApplicationNode 和 sse/notification-center 的事件投影。跨域协作使用稳定 ID、不可变快照、模块接口和可靠事件。
...
| 修改 Provider 或能力目录 | `domains/modelgateway/context.md` | 涉及应用消费行为时返回当前 Context |
| 修改 ApplicationRun | S1 product-spec | 涉及执行状态再读 task-center Context |
| 修改 Canvas 应用节点 | 当前 Context | 继续读取 workflow-canvas Context |

## 8. 当前状态

核心应用、Provider、ComfyUI、运行与跨域协作规则已有发布记录并正在实施。是否允许实现某一具体能力必须以 `RELEASE.md` 对应版本和 implementation gate 为准；S1 文件头版本可能滞后于后续发布记录。
