...
flowchart LR
  ...
  App["Application Platform<br/>ApplicationRun / Artifact refs"]
  Asset["Asset Library<br/>Artifact / AssetVersion / Representation"]
...
Task Center 发布 AtomicTask、TaskAttempt 和 Group/DAG 变化。Asset Library 发布 Artifact、登记和 AssetVersion Representation 处理变化；Application Platform 只发布 ApplicationRun 与 Artifact 引用关联，不发布竞争性的 Artifact 生命周期事实。
...
sequenceDiagram
  ...
  alt 保留范围内
    ...
    Web->>Facts: 重查任务、ApplicationRun、Artifact、AssetVersion 与 Asset
    Facts-->>Web: 当前事实
    Web->>Gateway: 使用新边界重连
  end
```
...
AssetVersion Representation 处理使用独立聚合事件：`asset_version.processing_started/progressed/ready/ready_with_warnings/processing_failed`。Artifact、AssetVersion 与 AtomicTask 不同聚合之间不保证严格顺序，Web 必须分别按 aggregate_version 幂等投影，不能由 AtomicTask 成功推断素材 ready。
