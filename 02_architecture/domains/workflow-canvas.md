...
CanvasVersion、CanvasRun 和 CanvasNodeRun 查询保留原始关系 ID，并组合一跳可读摘要。CanvasRun 创建时保存 Canvas 与不可变版本的非敏感摘要，旧数据缺失快照时以固定批次读取同域关系；DAGTaskGroup 与 AtomicTask 摘要通过 Task Center 受控批量能力获取，禁止跨领域私有表查询和逐行请求。关联资源不可见或缺失时只省略摘要。
