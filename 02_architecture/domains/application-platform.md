...
| 模块 | 架构职责 | 主要资源 |
| --- | --- | --- |
| `template` | 管理 ComfyUI 工作流模板与 SaaS API 模板、原始配置、SaaS 平台操作契约和引用关系 | `aiapp_app_templates` |
| `application` | 基于模板创建正式应用、维护应用配置、创建 AppRun 并委托 task-center 创建 TaskRun | `aiapp_applications`、`aiapp_application_runs` |
| `field-mapping` | 维护模板字段与应用入参之间的映射关系 | `aiapp_field_mappings` |
| `app-engine` | 管理用户自维护 AppEngine、SaaS 平台类型、支持能力、明文认证配置、健康状态和删除保护 | `aiapp_app_engines` |
...
- 依赖 `task-center` 周期性触发未停用 AppEngine 健康检测；应用平台负责连接平台、携带明文凭证、判断健康状态并写回。应用平台也支持直接使用 endpoint 和认证配置进行不持久化的临时健康检测。
...
sequenceDiagram
  ...
  App->>Engine: 校验 kind / SaaS 平台 / 能力 / 健康状态
...
- `kind=saas_api` 的模板保存 SaaS 平台类型、能力类型、操作标识和操作契约。
...
- SaaS API Application 从模板继承 SaaS 平台类型、能力类型和操作标识，并可以保存固化参数。
...
- `engine_type=saas_api` 的 AppEngine 必须保存 SaaS 平台类型，并可声明支持能力类型和健康检测配置。
- 使用 `app_engine_id` 的健康检测会写回已保存 AppEngine 的健康状态；使用 endpoint、认证方式和健康检测配置的临时检测只返回检测结果，不创建或更新 AppEngine。
- 未被 AppRun 引用的 AppEngine 可以删除；已被 AppRun 引用的 AppEngine 只能停用，不能物理删除。
- 运行 Application 时必须选择当前用户可访问、active、healthy、kind 匹配的 AppEngine；SaaS API 运行还必须匹配 SaaS 平台类型和能力类型。
