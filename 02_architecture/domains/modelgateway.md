...
本文在 application-platform v1.3.0 迁移的 Gateway 核心架构上补充 model-preferences 融合边界。产品语义以 `00_product/domains/modelgateway/product-spec.md` 为准，实现接口与数据结构以 `01_contracts/domains/modelgateway/` 为准。

Application Platform 保留 ApplicationExecutor、ApplicationRun 编排、运行快照和 Artifact 交付；model-preferences 保留展示偏好与默认 ProviderResource 引用；Model Gateway 统一提供 ProviderAccount/Resource、Registry、Binding、Adapter、发现、探测、健康和 `ResolveProviderTarget`/`ExecuteOperation` 边界。

## 1. 架构目标

- 将 ProviderType、ProviderAdapter、OperationExecutor 与易变的平台能力清单分离。
- 通过 Runtime Registry 维护稳定 `provider_type -> adapter_id` 和 `capability -> operation_executor_id` 内部映射，客户端只选择 Provider Type。
- 统一支持 `TargetSelection`，并在请求内派生不持久化的 `ResolvedExecutionRoute`。
- 以内置 YAML 与启动目录 YAML 作为 ProviderCapability 事实源，不创建数据库副本。
- 内置清单严格校验并保留 ID；目录文件失败只导致能力级降级，不覆盖内置能力。
- ProviderCapability 与 ComfyUI workflow 使用联合能力来源；ComfyUI 节点能力只由 ProviderAccount 当前 object_info 提供，不复制到工作流、校验、模板或运行。
...
flowchart LR
    ...
    T --> L
    T --> A["Provider / Provider Adapter Registry"]
    T --> O["OperationExecutor Registry"]
    PT["Provider Type Catalog"] --> T

    R --> B["ProviderAccountCapabilityBinding"]
    E["ProviderAccount"] --> B
    ...
    TR --> W["Worker"]
    W --> PE["TargetSelection"]
    MP["model-preferences<br/>Resource 引用与默认用途"] --> TS["TargetSelection"]
    TS --> MG["ResolveProviderTarget"]
...
```mermaid
flowchart TD
    A["注册 ProviderType"] --> B["注册 ProviderAdapter 与 OperationExecutor"]
    ...
    H --> I["保留 ID、kind 与 ProviderType/Adapter/Executor 校验"]
    I --> J{"文件全部通过"}
...
`kind=engine_binding` 只描述 ProviderType 的基础运行时身份，其 model、operation、variant 固定为空。ProviderCapability 均不描述：
...
flowchart LR
    ...
    C["Create comfyui engine"] --> T["Provider + binding transaction"]
    T -->|"binding failed"| X["Rollback engine"]
    T -->|"committed"| U
```

API Server 与 TaskWorker 都执行相同 bootstrap。唯一索引保证多副本只形成一条绑定；upsert 将 revision、enabled 和空 restrictions 恢复为当前内置事实。绑定管理 API 依据 ProviderCapability 的 binding_policy 派生 `system_managed` 并拒绝修改。ProviderAccount 删除由外键 cascade 清理绑定。
...
`ExecuteOperation` 接受 `ProviderExecutionGrant`、CapabilityDefinition、Operation 和标准参数。Gateway 校验目标类型、Registry 映射、principal、上下文签发者、能力、配置版本与有效期后，只在当前请求内产生 `ResolvedExecutionRoute`。Gateway 不为该结果建表、缓存第三份模型事实或提供 CRUD。

## 6. 数据归属

| 对象 | 事实源 | 是否持久化 |
| --- | --- | --- |
| CapabilityDefinition / ProviderType / Adapter / Executor | `runtime-registry.yaml` 只读契约与运行时注册表 | 否 |
| ProviderCapability | 启动目录 YAML 与进程内只读注册表 | 否 |
| ProviderCapabilityLoadResult | 当前进程加载诊断 | 否 |
| ProviderAccount | modelgateway 数据库合同中的兼容表 | 是 |
| ComfyUI Provider object_info | modelgateway 数据库合同中的 ProviderAccount 一对一当前目录 | 是，仅当前一份 |
| ProviderAccountCapabilityBinding | modelgateway 数据库合同中的兼容表 | 是 |
| ModelPreference / DefaultModelPreference | model-preferences 数据库合同 | 是，不进入 Gateway |
| provider-execution-grant | model-preferences 请求级签发结果 | 否 |
| ProviderExecutionGrant / ResolvedExecutionRoute | Gateway 请求级执行对象 | 否 |
...
ApplicationRun 创建时保存 Application、ApplicationVersion、ApplicationTemplateVersion、ProviderCapability 与非敏感 ProviderAccount 摘要；旧数据缺少 Gateway 快照时只能通过 Model Gateway 受控投影读取。Application 详情通过 Application Platform 分页接口读取持久化运行历史，默认 `created_at desc`。AtomicTask 摘要通过 Task Center service 边界批量读取。查询不跨领域私有表，不返回 Provider 鉴权、object_info、任务参数或输出；关联资源不可见或缺失时只省略摘要并保留原 ID。

ComfyUIWorkflow 不维护版本树或 lifecycle。每次导入生成新资源，只保存源文件、API Workflow 和元数据，不选择或保存 ProviderAccount，也不读取 object_info；单文件由服务端识别 visual 或 API 来源，visual 来源显式指定健康 ComfyUI 实例并使用其当前目录转换后才形成 API 执行事实。节点、候选项和依赖按目标实例当前目录即时计算，输出候选的 extractable 只来自 `object_info.output_node=true`。兼容性校验只保存独立诊断；每次模板转换直接选择目标实例实时校验，并把 API Workflow 和模板契约深拷贝到新的首个 draft ApplicationTemplateVersion，工作流不保存转换状态或转换历史。

WorkflowTestRun 归 application-platform 所有，通过 Task Center 创建 `submit -> poll -> collect_preview` DAG。创建时固定输入覆盖和 `node_id + output_index` 输出候选选择；collect_preview 将选择按 node_id 归并，只读取对应节点的轻量预览。输出正文不落库，预览经 ProviderAdapter 使用服务端 output ID 受控代理。

WorkflowTestRun 同时保存本次 ProviderAccount 的非敏感身份快照、输入参数覆盖快照和输出候选选择快照。列表投影默认只携带快照摘要、状态和进度，完整输入、输出选择、步骤与预览按 detail=true 或单条详情读取；历史配置再次运行仍通过标准创建链路重新校验并创建新的 DAG。
...
sequenceDiagram
    ...
    participant Provider as ProviderAccount
    ...
    Worker->>Gateway: ExecuteOperation(TargetSelection)
...
sequenceDiagram
    ...
    participant model-preferences as model-preferences
    participant Gateway as Model Gateway
    participant Provider as External Provider

    Chat->>model-preferences: 读取 ProviderResource 引用
    model-preferences-->>Chat: provider_resource_id
    Chat->>Gateway: ResolveProviderTarget + ExecuteOperation(grant)
    Gateway->>Gateway: 校验上下文并派生 ResolvedExecutionRoute
    Gateway->>Provider: Adapter + OperationExecutor
    Provider-->>Gateway: 结果或错误
    Gateway-->>Chat: 归一化输出或错误
```

Gateway 不查询 model-preferences 私有表，不创建或更新其展示偏好或 GenerationRun；Gateway 自身持久化 ProviderAccount/Resource 健康事实并发布事件。
...
- Provider Type 未注册、ProviderAccount/Resource 停用、健康不合格或配置版本不匹配：在调用 Provider 前拒绝，不回查 model-preferences 私有表。
...
## 10.1 ProviderAccount 周期健康检测

Model Gateway 注册 `model-gateway.provider-health` ReconcileHandler，Task Center 以同名 system_key 原子确保唯一 SYSTEM RECONCILE TaskSchedule。计划使用固定可复用的内部 workflow definition `task_center_reconcile_controller` 版本 1，按六段 cron 启动轻量 runtime execution，不为每轮注册 definition。

巡检器以稳定 ProviderAccount ID 为 checkpoint，按 max_parallelism 分块读取已启用实例并直接探测，不创建 Planner DAGTaskGroup 或健康 AtomicTask。默认并发 16、每轮 1000 项、单实例 4 秒和整轮 5 秒；只在整块完成后推进 checkpoint。前一轮非终态时当前 ScheduleExecution 记录 `SKIPPED_OVERLAP`。

检测结果通过 ProviderAccount resource version 乐观更新，旧结果不得覆盖新事实。健康状态变化时在同一业务事务中更新 ProviderAccount 并写 outbox；状态未变时只更新最近检测时间与监控指标。
...
Model Gateway 注册 `application-platform.comfyui-object-info-refresh` ReconcileHandler，Task Center 以同名 system_key 原子确保唯一 SYSTEM RECONCILE TaskSchedule。默认计划每日 `03:00 UTC` 运行，以 ProviderAccount ID 为 checkpoint，只读取 `comfyui + enabled + online` 实例；其他实例计入轻量 skipped 统计但不创建逐实例执行记录。

刷新器通过 ProviderAdapter 请求 `/object_info`，完成 JSON 结构与最低节点定义校验后，在单个实例串行化边界内原子 upsert 一对一当前目录。失败不删除或部分覆盖旧目录；多副本和手动刷新并发时，较早请求不能覆盖较新成功结果。目录表只保存正文、ComfyUI 版本和 `refreshed_at`，不保存 checksum、尝试历史、版本号或状态机。
...
- 普通能力目录不暴露磁盘路径、完整加载失败详情或 Provider 凭证。
- 文件级加载诊断仅管理员可见。
- ProviderCapability 无任何写 API 或重新加载 API。
- ProviderAccount 认证配置按 Model Gateway S1/S2 权限边界管理；ProviderCapability 文件不得包含凭证。
- `TargetSelection` 只携带非敏感 ProviderAccount/Resource ID；Gateway 签发的不透明 `provider-execution-grant://` 不能由客户端伪造，客户端不能提交 Provider 地址、凭证明文、Adapter ID 或 Executor ID。
- 应用创建者可以发现 ProviderAccount 基础状态并读取可见 ComfyUI 实例当前 object_info；base URL 的可见性沿用 ProviderAccount 契约，auth_config、凭证和实例写操作仍由管理员权限保护。
