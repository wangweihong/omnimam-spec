...
- 依赖 `model-preferences` 提供 ProviderResource 引用、默认用途和展示偏好；健康与能力事实不在本域缓存。
- 依赖 `modelgateway` 解析目标并签发不透明 `provider-execution-grant://`，统一执行 Adapter 与 OperationExecutor；AI Chat 不直接持有 Provider 地址、凭证或专用客户端。
- Topic、Assistant 和 QuickPhrase 查询使用同域批量摘要与 Model Preferences 受控批量模型投影组合可读关系；Message 历史继续以保存的模型/助手快照为准，不用当前配置覆盖历史语义。
...
sequenceDiagram
  ...
  participant Model as model-preferences
  ...
  User->>Msg: 提交用户消息
  Msg->>Gen: 创建生成运行
  Gen->>Model: ResolveProviderTarget
  Model-->>Gen: 模型/能力/配置版本上下文
  Gen->>Gen: 固定 GenerationRun 快照
  Gen->>Gateway: ExecuteOperation(ProviderTarget)
...
- GenerationRun 固定 `provider_resource_id`、`capability_definition_id`、`provider_account_config_version` 和非敏感模型快照；Model Preferences 或 Gateway 后续变化不得改写历史。
...
- 只能由 Model Preferences 签发执行上下文；客户端构造的 Provider 地址、凭证、Adapter ID 或 Executor ID 不得进入 Gateway 调用。
