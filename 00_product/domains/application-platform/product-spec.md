...
API Server 启动后必须立即检测所有已启用实例，之后按全局配置周期重复检测，默认周期为 30 秒；配置为 0 时关闭周期检测，但管理员手动检测仍可使用。周期检测只访问 `enabled=true` 的实例，禁用实例保留最后一次健康事实。多个实例默认并发检测，单实例和整轮检测均不得超过 5 秒。
...
34. `BR-AIAPP-163`：API Server 必须在启动后立即并按默认 30 秒可配置周期并发检测已启用 EngineInstance，单实例和整轮最多 5 秒；每次成功保存的检测均更新检测时间，并将安全失败摘要持久化及返回。多副本以到期筛选和乐观锁尽力去重，旧结果不得覆盖新事实；禁用实例不检测，周期配置为 0 时仅关闭自动检测。
...
* `AC-AIAPP-041-04`：API Server 启动后立即检测所有已启用实例，随后按默认 30 秒周期重复检测；配置为 0 时不启动自动检测，禁用和未到期实例不产生外部请求。
