...
flowchart TD
    subgraph ONBOARD["一、能力接入"]
        ...
        C1["选择来源实例并导入 ComfyUI API Workflow"] --> C2["读取实例当前 object_info 并创建私有 ComfyUIWorkflow"]
        C2 --> C3["按目标实例当前目录解析候选项、依赖并完成兼容性校验"]
...
ApplicationRun 的每个标准输出先形成不可变身份的 `Artifact`。Artifact 至少保存运行发起用户、ApplicationRun、输出名、媒体类型、外部结果引用或可控内容引用、处理状态和登记状态；同一 `application_run_id + output_key` 只能形成一个 Artifact。

Artifact 处理状态归 application-platform 所有：
...
* `created`：已保存 Artifact 身份与外部结果引用，可控 `content_ref` 可暂时为空。
...
`preview_ready` 是处理阶段中的独立事实，可在 `processing` 期间发生，不代表 Artifact 已进入 `ready`。预览和缩略图只保存受保护资源引用，不保存长期公开 URL。
...
* 重复登记返回同一 UserAsset；
* 内容缺失、不可读取、所有者不一致或媒体信息非法时，Artifact 的 `registration_status` 进入 `failed` 并保存稳定错误码和失败详情；
* 登记失败不得回滚或改写 AtomicTask 终态，可以由 application-platform 独立重试。

Artifact 处理与登记是两个独立维度。只有 `processing_status=ready` 才能请求登记；登记状态为 `pending | registered | failed`。Artifact 每次处理状态、预览就绪或登记状态变化必须递增 `resource_version`，并在同一事务写入 outbox 或等价可靠事件记录，供 SSE 等投影消费者按版本幂等处理。
...
→ 服务端读取来源实例当前 object_info
→ 创建当前用户私有且不带版本树的 ComfyUIWorkflow
→ 按所选实例当前目录识别节点、连接、输入输出候选、基础约束和运行依赖
...
Application Platform 通过 `application-platform.comfyui-object-info-refresh` SYSTEM RECONCILE 计划每日刷新一次。默认计划使用六段 cron `0 0 3 * * *` 和 `UTC`，只扫描类型为 `comfyui`、`enabled=true` 且 `health_status=online` 的实例。停用、unknown、offline 或 degraded 实例直接跳过，不创建逐实例动作、TaskGroup 或 AtomicTask；轮次只保存 Task Center 允许的轻量扫描摘要。
...
当前目录使用 `refreshed_at` 计算新鲜度，不另存状态。目录不存在或距最近成功刷新超过 48 小时时视为不可用于导入、解析、校验、转换、模板发布、RuntimeFormSchema 和运行；只读接口仍可返回最后一次成功目录并明确 `stale=true`，便于管理员诊断和手动恢复。实例未启用或不健康时，即使存在未过期目录，也不得用于需要实际执行能力的操作。
...
导入必须提供 API Workflow，并选择一个类型为 `comfyui` 的来源 `ApplicationEngineInstance`。普通 Workflow 可选，只用于节点位置、连线和标题等可视化展示，不参与执行。服务端必须读取来源实例当前、未过期的 `object_info` 完成基础结构校验，但只保存工作流文件和来源实例引用，不在工作流中保存目录正文、checksum 或派生解析缓存。客户端不得提交 `object_info` 作为能力事实。

应用创建者可以只读查询允许用于工作流导入和校验的 EngineInstance 标识、名称、类型、启用状态和健康状态，但不得读取鉴权配置、内部凭证或修改实例。实例创建、配置、健康检测和删除仍仅由管理员负责。

导入以原子方式完成：文件解析、来源实例校验、当前 `object_info` 可用性和基础结构校验任一步失败时，不产生 `ComfyUIWorkflow`。
...
导入后 API Workflow、普通 Workflow和来源实例不可修改。名称和描述可通过资源版本并发控制修改。工作流不提供归档、恢复或 lifecycle 状态；已经发布的归档/恢复接口废弃且不得继续作为新实现依据。
...
校验结果包含目标实例、可用时的 ComfyUI 版本、节点和依赖摘要、错误、警告及校验时间。只有当前目录可用且没有阻断错误的结果为 `compatible`；`incompatible` 和 `failed` 不能作为转换依据。历史 compatible 只表示校验当时的结果，不能替代转换、发布或运行时对当前目录的重新校验。

工作流成功转换后仍可对其他实例重新校验，但后续结果只用于诊断和运行兼容性评估，不得修改已生成的模板版本或 `workflow_contract_revision`。

---

### 5.9 一次性转换为 ApplicationTemplate

只有 API Workflow 已 ready、尚未转换且至少存在一条 `compatible` 校验记录的工作流可以转换。转换必须显式选择一条属于该工作流的兼容校验，并提供 `CapabilityDefinition`、模板元数据、暴露输入、固定参数、参数转换、输出提取和 Engine 选择约束。所选历史记录只用于确定目标实例和展示诊断；服务端必须在事务内使用该实例当前目录重新校验。
...
每个工作流最多成功转换一次，只用于创建新模板及其首个 draft 版本，不能追加到既有模板。转换完成后源内容继续只读；模板的后续调整、发布和演进全部通过 `ApplicationTemplateVersion` 机制完成。模板版本保存 API Workflow 和模板契约，但其当前可执行性始终取决于候选 EngineInstance 当前目录；目录变化不得改写模板版本，只能改变实时兼容性结果。
...
`visual_workflow` 来源保存原始画布，初始 `api_conversion_status=pending`；只有用户显式执行转换，且来源实例当前目录校验通过后，服务端才保存由开源 ComfyUI 图解析器生成的 API Workflow 并进入 `ready`。`api_workflow` 来源导入后直接为 `ready`，不得显示 Workflow 转 API 操作。源文件、来源类型和来源 checksum 导入后不可修改。

API Workflow ready 后，工作流所有者可以选择任意当前启用且健康、当前目录未过期的 ComfyUI EngineInstance 进行试运行。服务端在创建任务前读取目标实例当前目录完成兼容性和参数校验，并按所选节点参数生成不可变运行快照；试运行不得保存 `object_info` 正文或 checksum。不兼容、目录不可用、参数越界或 API 尚未 ready 时不创建任务。
...
每条 `ComfyUIWorkflowTestRun` 必须保存本次实际使用的 EngineInstance 非敏感快照和服务端校验后的参数覆盖快照。实例后续改名、停用或健康状态变化不得改写历史运行展示；列表默认只返回实例快照、参数覆盖数量和运行状态，参数值、任务步骤和输出等复杂数据仅在请求完整详情时返回。

用户可以从历史试运行详情发起“使用此配置再次运行”。该动作只把历史实例 ID 和参数覆盖快照带入新的试运行确认流程；服务端必须按当前实例可用性、当前 `object_info` 和参数约束重新校验，并使用新的幂等键创建独立 `ComfyUIWorkflowTestRun`，不得修改、恢复或直接复用历史任务。历史参数已经失效时，客户端必须阻止提交，直到用户移除或修正失效项。
...
试运行只保存图片、文本等输出的轻量描述。媒体正文不写入 Application Artifact 或 Asset；预览请求必须按服务端生成的 output ID，经所选 EngineAdapter 代理读取，不能接受客户端提供的 filename、路径、URL 或凭证。ComfyUI 清理输出后允许返回预览不可用，历史任务和输出描述仍保留。

前端在用户具有工作流转换与应用管理权限、工作流 API ready 且尚未转换时展示 ComfyUIWorkflow 转 ApplicationTemplate 入口；最终可转换性由服务端按目标实例当前目录重新校验。
...
ComfyUI 首个模板版本只能由未转换且 API ready 的 `ComfyUIWorkflow` 通过一次性转换动作创建。转换必须引用属于该工作流且状态为 `compatible` 的不可变校验记录，并在同一事务内按该记录对应实例的当前目录重新校验、创建模板、首个 draft 模板版本和转换关系。后续模板版本不再读取或修改源工作流，但发布和运行仍校验候选实例当前目录。
...
15. `BR-AIAPP-144`：ComfyUI 模板能力来自 API Workflow、候选 EngineInstance 当前 object_info、人工映射和模板约束，不创建 ProviderCapability，也不得借目录清单绕过工作流校验。
...
29. `BR-AIAPP-158`（deprecated，由 `BR-AIAPP-173` 替代）：旧规则把 active 生命周期和历史 compatible 记录作为转换权威条件。
...
38. `BR-AIAPP-167`：试运行参数只能覆盖 object_info 允许的标准字面量输入；连接、hidden、敏感字段、未知字段和越界值必须拒绝。
39. `BR-AIAPP-168`：试运行只保存轻量输出描述；预览按服务端 output ID 代理读取，不登记 Artifact/Asset，不接受任意上游路径或 URL。
...
41. `BR-AIAPP-170`：Application Platform 必须以 `application-platform.comfyui-object-info-refresh` SYSTEM RECONCILE TaskSchedule 每日刷新当前目录，只扫描 enabled、online 的 ComfyUI 实例并跳过其他实例；手动刷新使用相同资格校验，成功原子替换，失败保留最后一次成功目录，同一实例刷新必须串行化。
42. `BR-AIAPP-171`：ComfyUIWorkflow 只保存源文件、API Workflow、来源实例和元数据，不提供归档、恢复或 lifecycle；nodes、input-candidates、output-candidates、dependencies 必须接收目标 EngineInstance 并按其当前目录即时派生，不持久化解析缓存。
...
44. `BR-AIAPP-173`：模板转换必须在事务内按所选校验对应实例的当前目录重新校验；历史 compatible 只能作为选择与诊断依据，目录不可用、过期或当前不兼容时不得创建模板。
...
47. `BR-AIAPP-176`：object_info 不存在或距最近成功刷新超过 48 小时时视为 stale，不得用于导入、解析、校验、转换、模板发布、RuntimeFormSchema 或运行；只读目录仍可返回并显式标记 stale，实例 disabled 或非 online 时不得用于实际执行。
48. `BR-AIAPP-177`：Artifact 处理状态只能为 `created | transferring | processing | ready | failed | deleted`；只有 `ready` 可请求登记 UserAsset，`deleted` 不得级联删除已登记 UserAsset。
49. `BR-AIAPP-178`：Artifact 身份与 `application_run_id + output_key` 在创建后不可变；`content_ref` 在 `created/transferring/processing` 可为空，`ready` 和 `registered` 时必须存在。
50. `BR-AIAPP-179`：Artifact 预览就绪与处理状态独立；预览引用必须受控，不得保存或推送长期公开 URL、Provider 凭证或原始响应。
51. `BR-AIAPP-180`：Artifact 创建、处理状态、预览就绪和登记状态变化必须递增 `resource_version` 并同事务写可靠事件；处理或登记失败不反向改写已终态 AtomicTask。
...
* `AC-AIAPP-044-02`：文件非法、来源实例类型错误或当前 object_info 不存在、过期时不产生工作流资源。
...
* `AC-AIAPP-046-06`：转换以所选 compatible 记录定位目标实例，但必须在事务内读取该实例当前目录重新校验；目录 stale 或当前不兼容时不产生模板。
...
* `AC-AIAPP-048-01`：运行前即时兼容校验和参数校验失败时不创建 DAG。
...
* `AC-AIAPP-048-04`：历史列表显示运行实例快照和参数覆盖数量；详情显示实例快照、参数覆盖项、默认参数说明、三个任务步骤、错误和输出。
* `AC-AIAPP-048-05`：用户可将历史配置回填到新试运行弹窗；实例或参数失效时必须重新选择、移除或修正，确认后使用新幂等键创建独立任务。
...
* `AC-AIAPP-049-05`：目录缺失或超过 48 小时后，导入、解析、校验、转换、模板发布、RuntimeFormSchema 和运行均在调用 ComfyUI 前失败；只读目录可继续返回最后成功内容并标记 stale。

`US-AIAPP-050`：作为应用运行用户，我希望每个 Artifact 的传输、处理、预览就绪、可用性和 UserAsset 登记可被可靠追踪。
...
* `AC-AIAPP-050-04`：每次 Artifact 事实变更都产生可幂等投影的 outbox 事件，重复投递不导致处理状态回退或重复 UserAsset。
