...
试运行只保存图片、文本等输出的轻量描述。媒体正文不写入 Artifact 或 Asset；预览请求必须按服务端生成的 output ID，经所选 EngineAdapter 代理读取，不能接受客户端提供的 filename、路径、URL 或凭证。ComfyUI 清理输出后允许返回预览不可用，历史任务和输出描述仍保留。
