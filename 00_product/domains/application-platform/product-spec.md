...
| operationKey | string | 否 | SaaS 平台内具体接口能力标识；仅 kind=saas_api 时必填，例如 modelscope.image_generation |
| operationContract | object | 否 | SaaS 接口操作契约；描述请求参数、结果提取、健康检测相关约束的业务配置 |
...
#### operationContract(kind=saas_api)

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| method | string | 是 | 第三方接口调用方法或动作标识 |
| path | string | 否 | 第三方接口路径或资源标识 |
| requestMapping | object | 是 | 将应用输入和固化参数渲染为第三方请求的映射规则 |
| resultMapping | object | 否 | 将第三方响应映射为运行输出摘要或结果引用的规则 |
| cancelSupported | boolean | 是 | 该接口是否支持取消底层执行 |
| progressSupported | boolean | 是 | 该接口是否支持进度查询 |

ModelScope 生图模板是 `kind=saas_api` 的示例：`saasPlatformType=modelscope`、`capabilityType=image_generation`、`operationKey=modelscope.image_generation`，模板保存 ModelScope 生图接口的完整参数集合。后续 z-image、klein 等具体应用可以引用同一个 ModelScope 生图模板，并通过字段映射和固化参数选择具体模型。
...
| operationKey | string | 否 | SaaS 平台内具体接口能力标识；kind=saas_api 时继承自模板 |
...
| supportedCapabilityTypes | array | 否 | 该 SaaS 引擎支持的能力类型；engineType=saas_api 时用于运行匹配 |
| healthCheckConfig | object | 否 | 健康检测配置；不同 SaaS 平台和能力可以使用不同健康检测方式 |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| mode | enum | 是 | 检测方式：http_ping、api_call、none |
| path | string | 否 | 检测路径；适用于需要拼接 endpoint 的 HTTP 检测 |
| method | string | 否 | 检测请求方法 |
| expectedStatus | integer | 否 | 期望 HTTP 状态码 |
| payload | object | 否 | 检测请求载荷；仅保存小型结构化参数 |
...
| operationKey | string | 否 | SaaS 操作标识；kind=saas_api 时来自 Application |
...
* **BR-AIAPP-003** SaaS API 模板保存第三方平台某个接口的完整请求参数模板、参数 schema、结果提取路径和操作契约，并在创建模板时解析可映射变量。
...
* **BR-AIAPP-042** 当模板 `kind=saas_api` 时，必须填写 `saasPlatformType`、`capabilityType` 和 `operationKey`；当模板 `kind=comfyui` 时，不使用 SaaS 平台类型和 SaaS 能力类型参与运行匹配。
* **BR-AIAPP-043** SaaS API 模板中的 `operationContract` 用于表达第三方接口调用方法、请求映射、结果映射、取消支持和进度支持；不同第三方平台、不同能力可以拥有不同操作契约。
...
* **BR-AIAPP-045** 当应用基于 SaaS API 模板创建时，必须继承模板的 `saasPlatformType`、`capabilityType` 和 `operationKey`。
...
* **BR-AIAPP-028** 应用引擎用于承载应用运行所需的 ComfyUI 或 SaaS API 调用能力；AppEngine 本身只管理基础信息、支持能力和健康状态，应用运行结果由 AppRun 保存输出摘要或结果引用。
...
* **BR-AIAPP-034** 应用引擎健康检测需要连接应用引擎对应平台；当应用引擎配置了明文凭证时，健康检测必须携带对应凭证；不同 SaaS 平台和能力可以使用不同健康检测配置。
...
* **BR-AIAPP-047** 当 Application.kind 为 `saas_api` 时，运行所选 AppEngine 必须具有相同 `saasPlatformType`，并且 AppEngine 的 `supportedCapabilityTypes` 必须包含 Application 的 `capabilityType`。
...
* **BR-AIAPP-057** 已保存 AppEngine 健康检测通过 `appEngineId` 定位引擎，读取已保存的访问地址、认证方式、明文认证配置和健康检测配置，并在检测后写回健康状态、最近检测时间和不健康原因。
* **BR-AIAPP-058** 临时 AppEngine 健康检测用于创建或编辑引擎前验证连接参数，必须直接提交 `engineType`、`endpoint`、`authType` 和认证方式所需的 `authConfig`；该检测只返回 HealthCheckResult，不创建 AppEngine，不保存 endpoint、authConfig 或 healthCheckConfig，也不写回任何已保存 AppEngine。
* **BR-AIAPP-059** 当临时健康检测的 `engineType=saas_api` 时，可以提交 `saasPlatformType` 和 `healthCheckConfig`，用于选择第三方平台对应检测方式；缺少必需连接参数或认证配置不匹配时必须拒绝检测。
...
* **BR-AIAPP-052** TaskRun.input 必须包含 application_id、application_run_id、app_template_id、app_engine_id、kind、saas_platform_type、capability_type、operation_key 和 rendered_payload 等小型结构化上下文；大型图片、视频、音频结果不得直接写入 TaskRun 或 AppRun。
...
`kind=saas_api` 的模板详情还需要展示第三方平台类型、能力类型、操作标识和操作契约。ModelScope 生图模板可以作为平台化 SaaS API 模板示例，但页面不得将 ModelScope 写死为唯一平台。
...
普通用户、管理员和超级管理员可以查看自己的应用引擎列表、创建应用引擎、维护引擎名称、类型、访问地址、认证方式、明文认证配置和能力标签，停用不再可用的引擎，并删除未被 AppRun 引用的引擎。管理员和超级管理员还可以查看和管理普通用户的应用引擎。
...
用户也可以在创建或编辑 AppEngine 前直接提交 endpoint、认证方式、认证配置和健康检测配置执行临时连接检测。临时检测只返回检测结果，不保存凭证、不创建 AppEngine，也不改变任何已保存 AppEngine 的健康状态。

### US-AIAPP-012 创建 SaaS API 平台化模板和应用

普通用户可以创建 `kind=saas_api` 的模板，并选择第三方平台类型、能力类型和操作标识。平台创建模板前解析第三方接口参数，保存完整请求模板、参数 schema、操作契约和结果提取规则。用户可以基于该模板创建具体应用，并固化部分参数。例如，用户基于 ModelScope 生图模板分别创建 z-image 应用和 klein 应用。
...
当 Application.kind 为 `saas_api` 时，AppEngine 选择列表只展示同 `engineType=saas_api`、同 `saasPlatformType`、支持对应 `capabilityType` 且健康可用的引擎。不可用引擎可以展示为只读提示，但不得用于提交运行。
...
sequenceDiagram
  ...
  App->>App: 校验 kind / saasPlatformType / capabilityType / 健康状态
...
* **AC-AIAPP-011-06** 给定临时健康检测缺少 endpoint，或认证配置缺失、与 authType 不匹配，当用户提交检测时，系统应拒绝检测并提示健康检测配置非法或认证配置非法。
* **AC-AIAPP-012-01** 给定用户创建 `kind=saas_api` 模板，当未填写第三方平台类型、能力类型或操作标识时，系统应拒绝创建并提示 SaaS 平台配置不完整。
* **AC-AIAPP-012-02** 给定 ModelScope 生图模板创建成功，当用户基于该模板创建 z-image 应用和 klein 应用时，两个应用应继承同一 SaaS 平台类型、能力类型和操作标识，并可以保存不同固化参数。
...
* **AC-AIAPP-013-03** 给定 SaaS AppEngine 不支持 Application 的 `capabilityType`，当用户提交运行时，系统应拒绝并提示引擎能力不支持。
...
| app_engine_health_check_config_invalid | 应用引擎健康检测配置缺失或非法 |
| saas_platform_mismatched | SaaS Application 与 AppEngine 的第三方平台类型不一致 |
| app_engine_capability_unsupported | AppEngine 不支持 Application 所需能力类型 |
