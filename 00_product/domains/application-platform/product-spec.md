...
### 5.9 Workflow/API Workflow 双来源与试运行
...
`visual_workflow` 来源保存原始画布、导入时 `object_info` 快照和预检诊断，初始 `api_conversion_status=pending`；只有用户显式执行转换后，服务端才保存由开源 ComfyUI 图解析器生成的 API Workflow 并进入 `ready`。`api_workflow` 来源导入后直接为 `ready`，不得显示 Workflow 转 API 操作。源文件、来源类型和来源 checksum 导入后不可修改。

API Workflow ready 后，工作流所有者可以选择任意当前启用且健康的 ComfyUI EngineInstance 进行试运行。服务端在创建任务前重新读取目标实例 `object_info`，保存不可变兼容性校验，并按所选节点参数生成不可变运行快照。不兼容、参数越界或 API 尚未 ready 时不创建任务。
...
本阶段前端暂不展示 ComfyUIWorkflow 转 ApplicationTemplate 入口；既有接口、数据和后端能力保持兼容，后续产品阶段再恢复。
