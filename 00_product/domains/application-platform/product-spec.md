...
42. `BR-AIAPP-171`：ComfyUIWorkflow 只保存源文件、API Workflow、来源实例和元数据，不提供归档、恢复或 lifecycle；nodes、input-candidates、output-candidates、dependencies 必须接收目标 EngineInstance 并按其当前目录即时派生，不持久化解析缓存。output-candidates 只有在节点当前 `object_info.output_node=true` 时才可返回 `extractable=true`，普通中间节点端口不得作为应用或试运行的可提取输出。
