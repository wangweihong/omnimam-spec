...
Task Center 启动后必须立即创建健康 Planner TaskRun，之后按全局配置周期重复创建，默认周期为 30 秒；配置为 0 时关闭周期检测，但管理员手动检测仍可使用。Planner 只选择 `enabled=true` 且到期的实例并创建 PARALLEL TaskGroup；禁用实例保留最后一次健康事实。每个实例由独立 Atomic TaskRun 检测，组内最多并发 16 个，单实例最多 4 秒、整组最多 5 秒。
...
34. `BR-AIAPP-163`：Task Center 必须在启动后立即并按默认 30 秒可配置周期创建健康 Planner；Planner 为到期 EngineInstance 创建 PARALLEL TaskGroup，每个实例对应独立 Atomic TaskRun。单实例最多 4 秒、整组最多 5 秒；每次成功保存的检测均更新时间和安全失败摘要。多副本通过 TaskRun 幂等、Lease 和 resource version 防止旧结果覆盖；禁用实例不检测，周期配置为 0 时仅关闭自动检测。
