...
应用平台与执行编排用于连接 AI 计算能力和业务用户。平台将 ComfyUI 工作流、自建 GPU 集群、第三方 SaaS API 等底层能力抽象为业务用户可以直接使用的 Web 表单式应用。
将图像生成,图像编辑，视频生成等任务可以抽象成应用，其执行工作流为：
```text
输入参数【可选】 → 应用模板渲染 → 应用引擎异步执行 → 输出结果
```


应用分为两种类型：ComfyUI应用和SaaS应用.

ComfyUI API工作流是从ComfyUI平台导出的api json格式文件, 其通常为实现图像生成,图像编辑，视频生成，音频生成等任务的节点工作流。文件中其中通常有大量的节点，每个节点都有自己的参数和依赖关系。但对大部分业务用户来说，大部分节点和参数应当是透明的。
如图像生成,用户可能只关心输入的prompt、宽高比等参数，而不需要关系中间的节点和参数。平台支持用户上传api工作流文件，用户可以对工作流进行管理，比如自行根据api工作流的运行需求添加label selector类型标签用于指定最低/推荐环境需求配置，如GPU内存、显存、pytorch版本等。平台对环境需求配置不进行强行检测约束。但会影响平台智能调度到执行引擎的决策。
上传的工作流平台提供转换应用的功能,渲染成可视化节点工作流，从中选取用户感兴趣的参数，然后将api工作流转换成应用。一个工作流可以转换成多个不同的应用，每个应用都可以指定自己的参数。应用不深拷贝节点工作流，而是引用节点工作流。Comfyui AJSON应用只会记录可以填充的参数以及参数的默认值，以及引用的工作流。当工作流被引用时，其无法被删除。
应用需要在应用引擎上执行。如ComfyUI应用需要在ComfyUI类的应用引擎运行。ComfyUI应用引擎分为本地和云平台。本地可以是用户搭建的单点平台或者Comfyui弹性集群。云平台则是兼容ComfYui API JSON的云平台，比如runninghub,comfyCloud等。这些本地或平台必须能够保证支持应用绑定的工作流的所有节点和模型。平台不做强制限制，由用户自行保证。

很多第三方Saas平台（也包括用户搭建的vllm/llama-studio服务)都提供图像生成，图像编辑，视频生成等服务。这类平台通常提供特定的api接口和必要的参数，即可实现图像生成，图像编辑，视频生成，语音文本等功能。因此也可以将这些平台支持的api参数也抽象成应用。Saas应用不需要绑定工作流，因此可以直接创建saas应用，选择saas平台类型（比如modelscope，gpt),平台会自动提供modelscope平台参数，用户选择可配参数。Saas应用同样需要在应用引擎上运行。每个Saas类型的应用必须要指定其相应类型或者兼容api的平台。用户需要提前创建应用引擎，填写api密钥（自定义中转站平台还需要填充api服务地址）。

应用的运行都是异步的，用户发起应用执行请求后，应用引擎会在后台异步执行应用，用户可以在应用执行完成后在应用引擎任务列表或者任务中心查询应用执行结果。也可以设置webhook通过邮件、短信等方式通知用户应用执行结果。


应用的运行可以是命令式，也可以是声明式。
命令式的核心特征：用户对执行过程有完全控制权——指定目标、立即运行、失败即停。如用户指定要创建10张图片，用户提交执行请求后，应用引擎会在后台异步执行10张图片的创建任务，返回10张图片的失败或者成功的创建结果。
声明式核心特征：用户只描述"要什么"，平台负责"怎么达成"，自动协调直至完成。指的是用户指定要创建10张图片，用户提交执行请求后，应用引擎会在后台异步不停的执行创建任务，直到满足用户要求数量的图片成功创建出来。
应用的执行可以是串行或者并行。串行指的是应用引擎会按照用户指定的顺序执行任务，每个任务执行完成后才会开始下一个任务。并行指的是应用引擎会同时执行多个任务。用户要创建10张图片，如果是串行，则会一张一张向应用引擎发送运行要求，下一个执行必须等到上一个运行结果；如果是并行，则根据并行次数同时向应用引擎发送运行任务，当然应用引擎是否会排队任务取决于对应的平台。
用户可以创建运行策略，指定应用的运行模式，比如串行、并行、每次并行多少次、命令式、声明式、立即执行或者延迟运行，指定运行时间，周期性运行等。
可以创建运行任务，指定应用，运行策略，输入参数，输出参数，运行引擎等

应用从所属的角度上分为私有应用和公共应用。私有应用指的是用户自己创建的应用，其他用户是看不见的。公共应用则是全平台用户都可以直接使用的应用，当前仍然是得使用用户自己的应用引擎来运行。用户的应用选择上架到公共应用库，让其他用户可以使用。用户上架的应用需要经过管理员的评审测试通过后才能进入公共应用。管理员需要测试应用并提供正确的最低/推荐环境要求(comfUI应用).  公共应用无法被删除只能上架，下架。公共应用只是一个状态概念，其归属仍然属于某个用户。用户可以下架自己的应用，管理员也可以下架。管理员上架的管理员的应用不需要评审。公共应用可以被打分，记录其他用户的使用次数.


Comfyui应用引擎的标签非常重要，其作为运行任务智能调度，或者应用推荐引擎的依据。用户可以为应用引擎添加标签，比如comgyui引擎的vram_gb、cuda_versiond。平台会根据标签智能调度到合适的引擎。而saas应用引擎则在某些场景下，比如该saas平台只支持1k,2k图像生成，或者只支持文本生成能力等，这都对应用的调度有重大的影响.
