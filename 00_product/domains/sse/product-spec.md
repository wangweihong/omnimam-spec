...
12. SSE 推送制品创建事件。
13. 用户动作或 ApplicationVersion save policy 请求 asset-library 幂等登记 AssetVersion。
...
### 8.6 AssetVersion 处理事件

用于表示 Artifact 登记为 AssetVersion 后的 Representation 生成和周期修复结果。
...
Artifact 的处理状态由 asset-library 拥有：
...
新的制品记录已由 asset-library 根据受信 producer 请求创建。此时可以只有 producer 关联，内容尚未完成受控上传。
...
Artifact 因内容缺失、不可读、所有者不一致或媒体信息非法而登记失败。失败不得改写 AtomicTask 终态，asset-library 保留稳定错误码和失败详情，ApplicationRun 只消费只读投影。
...
## 15.11 AssetVersion 处理事件

AssetVersion 的处理事实由 asset-library 拥有。`processing_started/progressed` 表示首次 build DAG 或 backfill 修复正在推进；`ready` 表示所有必需 Representation 已就绪；`ready_with_warnings` 表示必需项可用但可选派生失败；`processing_failed` 表示必需校验或生成失败。

事件聚合必须使用 `aggregate_type=asset_version`、`aggregate_id=asset_version_id` 和单调 `aggregate_version`，并携带 `asset_id`、`asset_version_id`、处理状态、期望/完成/失败计数以及可选 `task_group_id/atomic_task_id`。周期 backfill 不产生专用用户事件，修复结果仍投影为上述 AssetVersion 事件。

Task Center 的 AtomicTask 成功只表达任务事实，前端不得据此直接把 AssetVersion 标记为 ready。Artifact、AssetVersion 和 AtomicTask 是不同聚合，事件之间不保证严格顺序。
...
Artifact 和 AssetVersion 事件使用 asset-library 已持久化 owner 解析：

```text
Artifact / AssetVersion
→ Asset Library owner_user_id
```
...
作为应用运行或素材用户，我希望 Artifact 的创建、传输、处理、预览就绪、Asset 登记结果以及 AssetVersion Representation 处理结果及时出现。
...
- `AC-SSE-003-04`：AssetVersion build 或 backfill 通过独立聚合事件更新 ready、ready_with_warnings 或 failed，AtomicTask 事件先后顺序不影响最终素材状态。
