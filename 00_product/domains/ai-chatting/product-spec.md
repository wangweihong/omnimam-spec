...
本特性的核心价值是把 Model Preferences 中的模型资源引用、助手预设、对话历史、消息生成、分支续聊、快捷输入、图片附件和翻译辅助收敛到一个可追溯、可持久化、可接入后端契约的产品入口。
...
classDiagram
  ...
  class Assistant {
    ...
    system_prompt
    is_system
    suggested_provider_resource_id
    use_suggested_model
    runtime_config
  }
...
它是 Model Preferences 对 Gateway `ProviderResource(kind=MODEL)` 的当前用户只读投影，用于展示、选择、健康状态、能力判断和生成时快照记录。AI Chat 不保存 endpoint、凭证或 Provider 私有配置。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| id | string | 是 | Gateway `ProviderResource.id` |
| displayName | string | 是 | 模型设置中的模型显示名称 |
| providerAccountId | string | 是 | Gateway ProviderAccount ID |
| providerType | string | 是 | ProviderType 稳定标识 |
| resourceKind | enum | 是 | 固定为 `MODEL` |
| capabilityDefinitionIds | array of string | 是 | Gateway 派生的只读可执行能力，例如 `text.chat_completion`、`text.translate`、`image.understanding` |
| enabled | boolean | 是 | Gateway 资源当前是否启用 |
| healthStatus | enum: unknown, healthy, unhealthy | 是 | Gateway 账号/资源健康投影 |
| unhealthyReason | string | 否 | 当前不可用原因摘要 |
| accountScope | enum | 是 | `USER` 或 `PLATFORM` |
...
| suggestedProviderResourceId | string | 否 | 助手建议的 Gateway MODEL Resource ID |
...
| providerResourceId | string | 是 | 当前话题使用的 Gateway MODEL Resource ID |
...
| providerResourceId | string | 是 | 当次使用的 Gateway MODEL Resource ID |
| capabilityDefinitionId | string | 是 | 当次 Gateway Operation 使用的能力 ID |
| providerAccountConfigVersion | integer | 是 | Gateway 账号配置版本 |
| providerResourceRevision | integer | 否 | Gateway 资源修订 |
| providerCapabilityRevision | string | 是 | Gateway 能力目录修订 |
...
- **BR-AICHAT-02** `Topic`、`Assistant` 和 `QuickPhrase` 默认归属于当前用户个人数据边界；模型配置归属于 `model-preferences`/`modelgateway`，AI 聊天只读取当前用户有权使用的 ProviderResource 引用，不维护模型事实源。
...
- **BR-AICHAT-12** 翻译能力依赖 Model Preferences 中当前用户的 `translation` 默认 MODEL Resource，并由 Gateway 重新校验 `text.translate`；没有合格目标时不得静默 fallback。
...
- **BR-AICHAT-15** 图片附件能力依赖 Gateway 对 ProviderResource 的当前能力；只有最终能力包含 `image.understanding` 的资源才可携带图片附件。
- **BR-AICHAT-16** 当前 ProviderResource 未启用、账号或资源非 `healthy`、能力修订失效或目标作用域不匹配时，发送、重新生成、编辑后重生成和图片附件发送均不得调用 Provider。
...
- **BR-AICHAT-24** 新话题或无当前模型上下文需要默认聊天模型时，必须读取 Model Preferences 的 `assistant.default` 默认 ProviderResource。
- **BR-AICHAT-25** Topic、Assistant 和助手级 QuickPhrase 响应必须保留 assistant/model ID，并同时返回当前用户权限边界内的一跳可读摘要。模型摘要由 Model Preferences 受控批量只读能力提供；Message 的 `model_snapshot`/`assistant_snapshot` 已是历史展示事实，父消息和 Generation 引用由当前 Topic 响应上下文解析，不再递归展开。
- **BR-AICHAT-26** AI Chat 不从 `feature_labels` 推断执行能力；普通聊天使用 `text.chat_completion`，翻译使用 `text.translate`，携带图片的聊天使用 `image.understanding`，且所需能力必须位于 Model Preferences 返回的 `capability_definition_ids`。
- **BR-AICHAT-27** 每次生成或翻译在调用 Provider 前必须通过 Model Gateway `ResolveProviderTarget(principal, selection, capability, execution_ref)` 校验作用域、启用、健康、能力和修订并签发不透明 `provider-execution-grant://`；随后以 Grant 调用 `ExecuteOperation`，AI Chat 不直接应用 Provider 协议或凭证。
- **BR-AICHAT-28** `GenerationRun` 创建时必须固定 ProviderResource ID、账号作用域、账号配置版本、资源修订、CapabilityDefinition 修订和非敏感模型快照；Grant 及其解析结果不得持久化，后续 Gateway 变化不得改写历史快照。
...
- 模型列表只读取 `model-preferences` 中当前用户自己的模型设置，AI 聊天不创建、同步、保存或删除模型。
- 新话题没有当前模型上下文时，默认模型来自 `model-preferences` 中当前用户的 `assistant.default` 配置。
...
sequenceDiagram
  ...
  participant UM as Model Preferences
  ...
  API->>MG: ExecuteOperation(ProviderTarget)
...
- 模型列表来自 Model Preferences 中当前用户自己的模型，支持按名称、服务商和只读 `capability_definition_ids` 过滤。
- AI 聊天不提供模型创建、同步、保存、删除能力；这些能力只属于 `model-preferences`。
- 已启用但健康状态为 `unhealthy` 的模型仍可在列表中展示，但必须显示异常原因并禁止选中。
- 助手可以配置建议模型；建议模型必须引用 `model-preferences.ProviderResource.id`。用户切换助手时保留当前模型选择，并提示可一键切换到助手建议模型，用户手动选择优先。
...
- 输入区翻译使用 `model-preferences` 中当前用户具备 `text.translate` 的默认翻译模型，把输入内容切换为目标语言译文，并允许恢复原文。
...
- 助手选择使用 Dialog。
- 模型选择使用 Dialog，并作为 Model Preferences 当前用户模型的只读选择入口，展示模型名称、服务商、可执行能力、健康状态和不可用原因。
