...
* 不直接管理具体模型加载逻辑
* 不直接解释 AppEngine 认证方式、明文凭证或健康判断规则
* 不直接保存大型图片、视频、音频二进制内容
* 不直接承担应用表单、模板、应用发布等应用中心职责

这些能力应由应用中心、应用引擎、素材库或具体业务模块负责。

任务中心可以周期性创建 `application-platform.app-engine-health-check` 系统维护任务，用于监听未停用 AppEngine 健康状态。任务中心只负责调度、TaskRun 生命周期、重试和失败记录；AppEngine 平台连接、明文凭证携带、健康判断和状态写回由应用平台负责。
