...
2. 周期性并发检测应用中心 EngineInstance，并汇总每轮结果。
...
TaskSchedule 保存一个 AtomicTaskTemplate、TaskGroupTemplate 或 DAGTaskGroupTemplate，并通过 cron 或单次 `runAt` 创建新的目标资源。
...
ScheduleExecution 保存 scheduledAt、triggeredAt、targetType、targetId、最终状态和跳过/失败原因。`scheduleId + scheduledAt` 全局唯一。
...
TaskSchedule 每轮创建 Planner DAGTaskGroup。Planner 枚举到期且启用的 EngineInstance，动态并行创建健康 AtomicTask，最多并发 16；单项最多 4 秒，整轮最多 5 秒。上一轮未终态时本轮记录 `SKIPPED_OVERLAP`。
