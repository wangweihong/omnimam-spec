...
本次联动修订日期为 2026-07-14，对齐 application-platform product-spec.md v0.7.0-draft。application-platform 的 Adapter 命名、模板版本对象和 Engine 选择方式仍是阻塞问题；任务中心不得提前替这些问题选定方案。两领域 S2 在后续同步前不得 release。
...
### 3.4 Worker 与应用平台执行能力分层原则

任务中心中的 Worker 与应用平台已经解析的执行能力、EngineInstance 必须明确分工。

```text
Worker 负责任务中心侧的领取、Attempt、Lease、进度、重试和取消生命周期。
应用平台已注册执行能力负责验证和渲染 ApplicationTemplate，并提供提交、查询、取消和结果归一化能力。
EngineInstance 表示实际 endpoint、凭证引用、地域、状态和能力绑定。
```

Worker 回答“谁执行并如何推进 TaskRun”；应用平台回答“应用模板和供应商协议如何执行”；EngineInstance 回答“连接到哪里以及当前实例能否执行”。

Worker 不直接解释应用模板、能力 Variant、供应商协议或 Engine 选择策略。Worker 领取 TaskRun 后调用应用平台已注册执行能力，并使用应用平台提供的模板与 EngineInstance 快照。应用平台以 `EngineAdapter` 负责平台级协议、`OperationExecutor` 负责具体 Operation，ProviderCapability 则由启动目录加载；任务中心不维护三者的事实副本。

---

### 3.5 协作式取消原则

任务取消不是强制杀死所有执行逻辑，而是由任务中心发出取消请求，Worker 与应用平台已注册执行能力协作响应。
...
* 提供任务查询与进度查询
* 支持故障扫描与恢复
* 为系统维护任务提供周期性调度，例如定期检查未停用 EngineInstance 健康状态
...
* 不直接管理具体模型加载逻辑
* 不直接解释 EngineInstance endpoint、凭证引用、能力绑定、应用模板映射或健康判断规则
* 不直接保存大型图片、视频、音频二进制内容
* 不直接承担能力注册、应用表单、模板版本、应用版本发布等应用平台职责

这些能力应由应用平台、素材库或具体业务模块负责。

任务中心可以周期性创建 `application-platform.engine-health-plan` 系统维护 TaskRun。Planner 由应用平台枚举到期实例并创建 PARALLEL TaskGroup；连接、凭证引用解释、健康判断和状态写回仍由应用平台负责。

---

## 5. 与应用平台的关系

任务中心是底层异步执行系统。应用平台负责能力、应用封装、按待确认策略解析 EngineInstance、模板渲染与业务输出；任务中心负责通用任务定义和运行生命周期。

### 5.1 应用平台

应用平台负责：

* CapabilityDefinition、ProviderCapability、CapabilityVariant 和 EngineCapabilityBinding
* Application、ApplicationTemplate 和 ApplicationVersion
* RuntimeFormSchema 与最终输入解析
* ApplicationRun
* 按 application-platform 已确认策略解析的 EngineInstance 和执行快照
* 应用输出定义、Artifact 和 Asset 关联
* 应用可见性与权限
...
### 5.2 已注册执行能力与 EngineInstance

应用平台已注册执行能力负责验证和渲染 ApplicationTemplate，并向 Worker 提供提交、查询、取消、恢复和收集标准输出的统一产品能力。其内部候选组件名称和分层不由 task-center 定义。

EngineInstance 是实际运行环境，保存 endpoint、credentialReference、地域、状态、并发限制和 EngineCapabilityBinding。任务中心只保存本次运行由应用平台解析出的 EngineInstance 引用与必要快照，不维护这些事实，也不决定采用固定或多 Engine 策略。
...
Worker 本身不决定具体业务任务如何执行。应用平台解释应用模板并使用已解析的 EngineInstance 执行；Worker 只维护任务协议。
...
```text
用户提交 ApplicationVersion 运行请求
  ↓
应用平台解析 RuntimeFormSchema 和执行环境
  ↓
应用平台创建 ApplicationRun 与不可变执行快照
  ↓
应用平台以 applicationRunId 和幂等键请求任务中心基于 AtomicTask(application.execute) 创建 TaskRun
  ↓
Worker 领取 TaskRun，创建 TaskAttempt 和 ExecutionLease
  ↓
Worker 调用应用平台已注册执行能力
  ↓
已注册执行能力渲染 ApplicationTemplate，并使用 EngineInstance 执行 ComfyUI / SaaS / LLM / TTS / 本地模型
  ↓
Worker 同步进度、Attempt、externalJobId 与结果引用
  ↓
任务中心更新 TaskRun 状态并产生带 resourceVersion 的事件
  ↓
应用平台幂等更新 ApplicationRun 投影
```

简化表示：

```text
ApplicationRun → TaskRun → TaskAttempt / Lease → Worker → 已注册执行能力 → EngineInstance → Result
```

应用运行 TaskRun 必须保存 `applicationRunId`。同一 `applicationRunId + idempotencyKey` 的重复创建请求返回同一 TaskRun，不产生第二次执行。任务创建成功后，所有状态、进度和结果事件都携带 `applicationRunId`，使应用平台无需依赖 Adapter 或 Operation 字段即可关联 ApplicationRun。`adapterKey`、`operationKey` 和 `operationVersion` 不属于 application.execute TaskRun 的必填路由信息；具体执行能力只从 ApplicationRun 不可变快照读取。

### 5.5 应用与画布任务映射

直接应用运行使用 AtomicTask，functionRef 固定为 application.execute。AtomicTask 描述如何调用应用执行入口，不代表某一次运行。

画布保存态由 workflow-canvas 负责。画布编译时生成 DAGFlowTask，ApplicationNode 对应引用 application.execute 的 DAGNode；执行 DAG 时创建根 TaskRun，每个应用节点创建子 TaskRun，并由 rootRunId、parentRunId 保留运行树。

运行对象关系为：

```text
ApplicationRun → TaskRun → TaskAttempt
CanvasRun → 根 TaskRun
CanvasNodeRun → 子 TaskRun → ApplicationRun
```

TaskRun 是状态、进度、重试、超时和取消的唯一事实源。ApplicationRun 与 CanvasNodeRun 只能使用 taskRunId 和更大的 resourceVersion 幂等投影，不得维护独立执行状态机。
...
系统维护任务也遵守同一转换链路。scheduler 可以按周期创建 `application-platform.engine-health-plan`、`asset.sha256_backfill` 等系统维护 TaskRun，但具体健康判断、素材扫描、业务数据写回仍由所属 domain 解释。任务中心只负责计划创建、排队、Attempt、Lease、重试、失败记录和状态事件。
...
作为业务用户，我希望在应用平台填写表单并提交运行请求，系统可以异步执行任务，并允许我查看运行进度和最终结果。

验收要点：

* 用户可以提交应用运行请求。
* 系统创建 ApplicationRun 和 TaskRun。
* 相同 ApplicationRun 和幂等键的重复请求返回同一 TaskRun，不产生重复执行。
* 用户可以查看任务状态。
* 用户可以查看任务进度。
* 任务完成后，用户可以查看输出结果。
* 任务失败后，用户可以查看失败原因。
* ApplicationRun 只按 TaskRun resourceVersion 幂等投影状态和结果。
* application.execute 的创建、状态、进度和结果事件均携带 applicationRunId。
...
* ApplicationNode 应引用 application.execute AtomicTask，并创建关联 ApplicationRun 的子 TaskRun。
* CanvasRun 对应根 TaskRun，CanvasNodeRun 对应子 TaskRun，运行树必须可追踪。
...
| maxAttempts    | 最大尝试次数                     |
| progress       | 当前进度                       |
| resourceVersion | 状态和结果版本；消费者据此幂等投影       |
| input          | 本次运行输入                     |
| output         | 本次运行输出摘要                   |
| lastError      | 最近一次错误摘要                   |
| applicationRunId | 关联 ApplicationRun；非应用任务为空 |
| applicationVersionId | 应用运行使用的 ApplicationVersion 快照 |
| applicationTemplateSnapshot | 应用运行使用的 ApplicationTemplate 必要快照；是否引用独立版本对象由 application-platform 确认 |
| executionCapabilityKey | 应用平台已解析执行能力的快照标识；不预设 Adapter 名称 |
| operationKey   | 应用运行使用的 Operation 快照         |
| capabilityTemplateVersionId | 已发布平台能力事实版本快照 |
| engineInstanceId | 应用平台按已确认策略解析的 EngineInstance |
| createdAt      | 创建时间                       |
| startedAt      | 首次开始时间                     |
| completedAt    | 完成时间                       |
| canceledAt     | 取消请求时间                     |
...
Worker 不直接代表业务能力本身。应用模板和供应商协议由应用平台已注册执行能力解释，运行实例能力由 EngineInstance 和 EngineCapabilityBinding 描述。
...
sequenceDiagram
    ...
    participant Task as 任务中心
    participant Worker as Worker
    participant Exec as 应用平台执行能力
    participant Engine as EngineInstance
    participant Asset as 素材库

    User->>App: 提交 ApplicationVersion 运行请求
    App->>App: 解析 RuntimeForm 和执行环境
    App->>App: 创建 ApplicationRun 与执行快照
    App->>Task: 创建 application.execute TaskRun
    ...
    Worker->>Exec: 传入模板、Engine 和参数快照
    Exec->>Engine: 渲染并调用 endpoint
    Engine-->>Exec: 返回进度 / 外部任务状态
    Exec-->>Worker: 返回标准化进度
    Worker->>Task: 上报 progress / heartbeat / renew lease
    Engine-->>Exec: 返回最终结果
    Exec-->>Worker: 返回标准化输出引用
    Worker->>Task: complete TaskRun
    Task->>App: 同步带 resourceVersion 的 TaskRun 事件
    App->>App: 幂等更新 ApplicationRun 投影
    App->>Asset: 登记 Artifact 为当前用户 Asset
```
...
## 11. Worker、应用平台执行能力与 EngineInstance 协作规则
...
* 重试前优先使用 externalJobId 恢复已有外部任务

---

### 11.2 应用平台已注册执行能力负责的事情

应用平台已注册执行能力负责：

* 验证 ApplicationTemplate 快照；
* 将 ApplicationRun 业务输入渲染为底层执行请求；
* 使用应用平台已经解析的 EngineInstance；
* 提交、查询、取消和恢复执行；
* 把供应商结果转换为标准 Artifact；
* 把供应商错误转换为应用平台和任务中心可识别的标准错误。

当前模型、最大分辨率或最大时长等事实来自应用平台启动时加载且状态为 `available` 的 ProviderCapability 和 CapabilityVariant，不由 Worker 或执行组件自行维护。能力不可用时，应用平台必须在创建或执行 TaskRun 前失败，任务中心不得绕过能力校验。

### 11.3 EngineInstance 负责的事情

EngineInstance 表示实际连接环境，提供 endpoint、credentialReference、地域、网络配置、并发限制、激活状态、健康状态和 EngineCapabilityBinding。它不提供任务中心状态机，也不决定 Worker 的 Lease 或重试。

### 11.4 三方边界判断

```text
领取任务、维护 lease、上报进度和写回结果：Worker。
解释应用模板、供应商协议并形成标准输出：应用平台已注册执行能力。
表达连接位置、凭证引用和实例能力：EngineInstance。
```
...
* 应用平台按 resourceVersion 幂等更新 ApplicationRun 投影
* 前端实时刷新任务进度
* 审计日志记录
* 通知用户
* 触发下游流程
...
Task history
基础事件
与 ApplicationRun / 应用平台执行能力 / EngineInstance 的运行链路
```
...
8. BR-TASK-020：Worker 不决定应用模板、供应商协议或 Engine 选择策略，这些由应用平台负责。

---

### 19.4 应用平台执行能力与 EngineInstance 协作规则

1. BR-TASK-021：应用平台已注册执行能力负责 ApplicationTemplate 快照的验证、渲染、执行入口和标准输出；内部组件命名由 application-platform 决定。
2. BR-TASK-022：Worker 负责调用应用平台已注册执行能力并同步 TaskRun 状态。
3. BR-TASK-023：应用平台负责具体供应商鉴权、请求、状态和错误协议，任务中心不解释这些协议。
4. BR-TASK-024：Operation 支持取消时，应用平台已注册执行能力应向 Worker 暴露取消能力。
5. BR-TASK-025：应用平台执行能力返回的大型结果应写入素材库或对象存储。
6. BR-TASK-026：Worker 将标准结果摘要和引用写回 TaskRun。
...
2. BR-TASK-038：Worker 收到取消请求后，应调用应用平台已注册执行能力尝试取消底层任务。
...
1. BR-TASK-051：直接应用运行使用 AtomicTask(application.execute)，AtomicTask 只描述任务定义。
2. BR-TASK-052：画布编译为 DAGFlowTask，ApplicationNode 对应引用 application.execute 的 DAGNode。
3. BR-TASK-053：ApplicationRun 必须关联一个 TaskRun；每次 Worker 领取或重试必须创建新的 TaskAttempt。
4. BR-TASK-054：画布根运行使用根 TaskRun，节点运行使用带 parentRunId 和 rootRunId 的子 TaskRun。
5. BR-TASK-055：TaskRun 是执行状态唯一事实源，ApplicationRun 和 CanvasNodeRun 只能按 resourceVersion 幂等投影。
6. BR-TASK-056：应用平台必须在创建 TaskRun 前解析并快照 ApplicationVersion、ApplicationTemplate 及按已确认策略选择的 EngineInstance；是否存在独立模板版本对象由 application-platform 决定。
7. BR-TASK-057：externalJobId 属于 TaskAttempt；重试前应优先恢复已存在的外部任务。
8. BR-TASK-058：取消 ApplicationRun 或 CanvasRun 必须转换为 TaskRun 取消请求；DAG 根任务取消应级联未完成子任务。
9. BR-TASK-059：平台能力不匹配按标准 TaskRun 失败处理，能力修正事项由 application-platform 创建。
...
6. BR-TASK-068：TaskGroup 根运行由任务中心展开和聚合，不得被 Worker 直接领取；实际执行只发生在 AtomicTask 子运行。
7. BR-TASK-069：动态 TaskGroup 子运行必须引用定义 children 白名单，展开必须原子、可幂等，并限制为最大 16 层和 10000 个 TaskRun。
8. BR-TASK-070：PARALLEL TaskGroup 必须在多副本领取竞争下严格遵守 maxParallelism，并聚合子运行进度与终态。
9. BR-TASK-071：取消、整体超时和组级重试必须级联到子运行；PARALLEL 默认仅重试失败子运行。
10. BR-TASK-072：AppEngine 周期健康检测由 `application-platform.engine-health-plan` 枚举业务事实，再创建 PARALLEL TaskGroup；任务中心不得直接读取 EngineInstance。
...
* Lease 续约和过期规则
* TaskAttempt 错误结构
* ApplicationRun、CanvasNodeRun 与 TaskRun 关联模型
* Worker、应用平台已注册执行能力与 EngineInstance 协作契约
* 审计日志结构
* 监控指标定义
...
Worker 调用应用平台已注册执行能力
  ↓
执行能力渲染 ApplicationTemplate 并使用 EngineInstance 执行业务能力
  ↓
Worker 写回状态和结果
```
...
* Worker 负责任务中心执行协议。
* 应用平台已注册执行能力负责应用模板验证、渲染、具体供应商协议和标准执行入口；其内部组件命名待 application-platform 确认。
* EngineInstance 表示实际 endpoint、凭证引用、状态和能力绑定。
* Watchdog 负责故障扫描和恢复。
* 应用平台通过 ApplicationRun 与任务中心打通用户可见的运行链路。
