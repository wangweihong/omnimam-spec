...
### 3.4 Worker、ProviderAdapter 与 AppEngine 分层原则

任务中心中的 `Worker`、应用平台中的 `ProviderAdapter` 和 `AppEngine` 需要明确分工。

```text
Worker 负责任务中心侧的执行生命周期。
ProviderAdapter 负责业务能力侧的具体执行逻辑。
AppEngine 负责运行实例配置和运行状态。
```
...
ProviderAdapter 回答：具体平台协议如何调用、轮询、取消和标准化结果？
AppEngine 回答：调用哪个 endpoint、使用什么认证和运行能力？
```

Worker 不应直接承担复杂业务能力封装。Worker 领取 TaskRun 后，根据执行上下文调用已注册 ProviderAdapter；ProviderAdapter 使用已解析 AppEngine 的 endpoint、认证和能力快照完成具体执行。

---

### 3.5 协作式取消原则

任务取消不是强制杀死所有执行逻辑，而是由任务中心发出取消请求，Worker 和 ProviderAdapter 协作响应。
...
* 提供任务查询与进度查询
* 支持故障扫描与恢复
* 为系统维护任务提供周期性调度，例如定期监听未停用 AppEngine 健康状态
...
* 不直接管理具体模型加载逻辑
* 不直接解释 AppEngine 认证方式、明文凭证、custom_http 配置、SaaS 平台能力矩阵或健康判断规则
* 不直接保存大型图片、视频、音频二进制内容
* 不直接承担应用表单、模板、应用发布等应用中心职责

这些能力应由应用中心、应用引擎、素材库或具体业务模块负责。

任务中心可以周期性创建 `application-platform.app-engine-health-check` 系统维护任务，用于监听未停用 AppEngine 健康状态。任务中心只负责调度、TaskRun 生命周期、重试和失败记录；AppEngine 平台连接、明文凭证携带、custom_http 配置解释、平台预置健康检测策略、健康判断和状态写回由应用平台负责。

---

## 5. 与应用平台、ProviderAdapter 和 AppEngine 的关系

任务中心是底层异步执行系统，应用平台负责用户可见的应用封装和引擎路由，ProviderAdapter 负责平台执行协议，AppEngine 表达运行实例。
...
### 5.2 ProviderAdapter 与 AppEngine

ProviderAdapter 是受控代码扩展点，负责封装具体执行协议，例如 ComfyUI、Seedance、OpenAI 或其他 SaaS API 的提交、轮询、取消、结果收集和错误标准化。

AppEngine 是运行实例配置，例如：
...
* 上报任务进度
* 响应取消请求
* 调用对应 ProviderAdapter
* 写回成功结果
* 写回失败结果
* 记录外部任务 ID

Worker 本身不决定具体业务任务如何执行。具体执行逻辑由 ProviderAdapter 决定，AppEngine 只提供实例配置。
...
应用中心渲染应用模板
  ↓
应用平台校验用户指定引擎或自动路由 AppEngine
  ↓
任务中心创建 TaskRun
  ↓
Worker 领取 TaskRun
  ↓
Worker 调用 ProviderAdapter
  ↓
ProviderAdapter 使用 AppEngine endpoint 执行 ComfyUI / SaaS / LLM / TTS / 本地模型
  ↓
Worker 同步进度与结果
  ↓
任务中心更新 TaskRun 状态
...
```text
AppRun → TaskRun → Worker → ProviderAdapter → AppEngine endpoint → Result
```
...
| output         | 本次运行输出摘要                   |
| lastError      | 最近一次错误摘要                   |
| adapterKey     | 应用运行使用的 ProviderAdapter 快照   |
| operationKey   | 应用运行使用的 Operation 快照         |
| operationVersion | Operation 契约版本快照            |
| requestedEngineId | 用户指定引擎；自动模式为空             |
| resolvedEngineId | 实际占用的 AppEngine               |
| createdAt      | 创建时间                       |
| startedAt      | 首次开始时间                     |
| completedAt    | 完成时间                       |
| canceledAt     | 取消请求时间                     |
...
Worker 不直接代表业务能力本身。业务协议由 ProviderAdapter 描述，运行实例能力由 AppEngine 描述。
...
sequenceDiagram
    ...
    Adapter->>Engine: 调用平台接口
    Engine-->>Adapter: 返回进度 / 外部任务状态
    Adapter-->>Worker: 返回标准化进度
    Worker->>Task: 上报 progress / heartbeat / renew lease
    Engine-->>Adapter: 返回最终结果
    Adapter-->>Worker: 返回标准化结果
...
## 11. Worker、ProviderAdapter 与 AppEngine 协作规则
...
* 接收 cancel 请求
* 调用 ProviderAdapter
* 将 ProviderAdapter 的标准结果写回任务中心
* 将 ProviderAdapter 的标准错误写回 TaskAttempt
* 将外部任务 ID 写入 TaskAttempt
* 重试前优先使用 externalJobId 恢复已有外部任务

---

### 11.2 ProviderAdapter 负责的事情

ProviderAdapter 负责具体业务能力执行，包括：
...
### 11.3 AppEngine 负责的事情

AppEngine 只提供 endpoint、认证配置、运行版本、支持 Operation、节点/模型能力、健康状态和并发状态。AppEngine 不提供执行方法。
...
```text
如果它描述“平台协议怎么调用”，它是 ProviderAdapter。

如果它描述“能力在哪里、实例支持什么”，它是 AppEngine。
...
| ComfyUI workflow 执行适配逻辑          | ProviderAdapter |
| 领取 TaskRun 并调用 ComfyUI Adapter 的进程 | Worker    |
| OpenAI API Key、endpoint、model 配置 | AppEngine |
| OpenAI 请求、结果和错误转换逻辑              | ProviderAdapter |
| 调用 OpenAI Adapter 并同步结果的任务工作器    | Worker    |
...
* 将业务输入传递给对应业务模块或 ProviderAdapter。
...
8. BR-TASK-020：Worker 不决定具体业务执行逻辑，具体执行由 ProviderAdapter 负责。

---

### 19.4 ProviderAdapter 与 AppEngine 协作规则

1. BR-TASK-021：ProviderAdapter 负责具体平台执行协议，AppEngine 只提供运行实例配置和状态。
2. BR-TASK-022：Worker 负责调用 ProviderAdapter 并同步状态。
3. BR-TASK-023：ProviderAdapter 应按 Operation 声明提供进度查询能力。
4. BR-TASK-024：Operation 支持取消时，ProviderAdapter 应向 Worker 暴露取消能力。
5. BR-TASK-025：ProviderAdapter 返回的大型结果应写入素材库或对象存储。
...
2. BR-TASK-038：Worker 收到取消请求后，应调用 ProviderAdapter 的取消能力。
...
* ProviderAdapter、AppEngine 与 Worker 协作契约
* 审计日志结构
* 监控指标定义
...
Worker 调用 ProviderAdapter
  ↓
ProviderAdapter 使用 AppEngine endpoint 执行业务能力
  ↓
Worker 写回状态和结果
```
...
* Worker 负责任务中心执行协议。
* ProviderAdapter 负责具体业务能力执行。
* AppEngine 只负责运行实例配置和状态。
* Watchdog 负责故障扫描和恢复。
