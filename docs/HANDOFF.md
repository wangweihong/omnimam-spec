...
- 状态：`spec-v1.22.0` 已发布；release commit `edcdbcebf8daecec8eaefd129338e829512b00fe`、annotated tag 和 `master` 均已推送到 `origin`。
...
- 已创建 release commit `edcdbcebf8daecec8eaefd129338e829512b00fe` 和 annotated `spec-v1.22.0` tag，并推送 branch/tag。

## 当前进行中

- 无；下游 server 可以更新 SSOT pin 并实施。
...
- release commit、tag 和远端 branch 已核对；下游尚需 pin 与实现验证。
- spec worktree 既有未跟踪 `archive/`、`docs/identity_fix.md`、`设计图/`，本任务不得提交或修改。

## 未完成事项

- 下游 server pin 到 `edcdbcebf8daecec8eaefd129338e829512b00fe` 并实施。
- 下游 server 更新 SSOT pin 后实施；devops 完成幂等用户和 PAT bootstrap。

## 推荐下一步

在 omnimam-server 更新 `ssot` 与 `SSOT_VERSION`，核对 release gate 后实施独立 GitLab domain。
