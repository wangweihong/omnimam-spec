...
- Notification 使用 notification resource_version，未读计数使用 recipient counter version；两个聚合版本不可比较。通知 UserEvent 只提示收件箱变化，不能覆盖 Notification REST 事实。
...
- 以 UserEvent 代替审计日志、任务历史、Artifact 生命周期、UserAsset 或 Notification 事实。
