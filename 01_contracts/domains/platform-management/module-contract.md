...
- Identity 不得缓存无法与平台版本关联的认证策略；配置更新事件只用于失效和刷新提示，不能扩大授权。
...
- 审计摘要只包含 ID、来源、操作、目标引用、结果和创建时间，不展开 detail、凭据、Token、原始请求或业务正文。
...
- 受控 domain 使用 `platform.audit.record` 提交 `source_domain`、`source_module`、PrincipalContext 摘要、目标、结果、原因、request/trace ID、幂等键和最小 detail。
- Platform 服务端校验调用服务主体、委托用户、来源登记、字段长度、禁止字段和幂等键；调用方不能指定伪造的主体或绕过审计范围。
- `platform_audit_logs` 只能追加，业务 API 不提供更新或删除；同一幂等键返回同一审计记录摘要。
- detail 必须脱敏且有界；不得包含密码、完整 Access/Refresh Token、Secret、凭据哈希、Authorization、原始请求 payload、Provider 响应、文件路径或大型正文。
- 登录、密码、Token、授权、服务账号、跨 owner 和认证配置等敏感操作在审计写入失败时必须 fail closed；普通只读查询不因不存在审计动作而失败。
...
- Identity 事件中的 `platform-audit` 消费者只能使用事件中的最小非敏感上下文；不能读取 Identity 私有表补齐审计记录。
...
- AuditLog 列表固定分页，支持 source_domain、source_module、principal、action、result 和时间范围过滤。
- 查询结果只返回权限裁剪的固定字段和有限 detail；不可见记录按稳定业务错误返回，不泄露存在性。
- 跨 domain 审计列表不递归展开目标资源；保留稳定 ID，必要的名称和状态由目标 domain 提供一跳摘要。

## 8. 实施门禁

- 本合同完成后仍需用户确认并登记 `RELEASE.md`；未 Release 前只能用于草稿讨论、实现评估和合同审查。
- 正式实现前必须确定 Identity 服务主体调用 Platform 的认证、授权、幂等和 fail-closed 传递方式；实现不得直接访问平台数据库。
