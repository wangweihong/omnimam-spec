...
| --- | --- | --- | --- |
| `agent.invocation.execute` | SERVICE | agent | AgentInvocation |
...
- `agent.invocation.execute@1.0` 的 arguments 只能包含 Agent、Session、Invocation、RuntimeBinding、Invocation 类型、短期 `agent-invocation-grant://` 和 `provider-execution-grant://`、预期资源版本和可选 runtime/event 恢复游标。Task Worker agent-executor 仅可在当前 Attempt 内以受信身份解析授权引用，读取 Invocation/Session/消息上下文、Runtime Endpoint 和 Gateway ModelAccessSpec；CODING claims 还固定 StudioApplication/Workspace、base Revision/CommitSHA、Blueprint version 和 prompt kind，但不包含源码正文工具授权或明文 Git token。解析结果不得进入 Task arguments/result、事件、日志或持久化 Task 投影。
- `agent.runtime.ensure@1.2` 是唯一 ACTIVE 版本；`1.0` 与 `1.1` 以原 schema/digest 保留为 RETAINED，并继续使用 `agent-model-access-grant://`。`1.2` 改用 Model Gateway `provider-execution-grant://`，且 Coding Agent arguments 必须继续携带 `appstudio-runtime-git-access://` opaque secret reference；Platform Agent 不要求 Git access 字段。Task、日志和业务投影不得包含 grant 解析结果、clone URL、用户名或 token。
...
- reconciler 对 execution 丢失及 SUCCESS、FAILED、TIMEOUT、CANCELED 终态都调用已注册 observer；Agent observer 按当前 Task/资源版本投影，服务 context 取消时 reconciler 必须退出。
...
- 来源领域在完成既有权限校验后提供主业务资源：AppStudio 任务统一关联 StudioApplication，Agent 任务关联 Agent，应用运行关联 Application，Canvas 任务关联 Canvas，素材任务关联 Asset 或 Artifact，GitLab Pipeline 关联 GitLabProject。名称只保存当时可可靠取得的非敏感值，不回查可变名称改写历史。
...
- modelgateway 注册 `model-gateway.provider-health` ReconcileHandler。其 SYSTEM TaskSchedule 直接分批探测 ProviderAccount/ProviderResource，不创建 Planner DAGTaskGroup 或健康 AtomicTask；状态变化由 modelgateway 在同一事务中更新投影并写 outbox。
...
- 日志 envelope v2 的 `stage` 只允许 PREPARE、EXECUTE、WAIT、POST_PROCESS、CLEANUP；`context` 只允许 attempt_no、duration_ms、progress、item_count、output_count、operation、external_status、retryable。所有日志字符串字段在 Worker 写入和 Task Center 读取边界双重脱敏、单行化并限制为 4096 字节；禁止 DEBUG、Stack、Cause、RawCode、自动捕获全局进程日志、鉴权信息、凭证、Provider 原始响应、任意 URL、文件路径或大型正文。
