...
- 负责普通用户、管理员和超级管理员在模板、应用和字段映射上的可见性与管理边界。
- 普通用户只能管理自己的模板和应用；管理员和超级管理员可以管理全部用户的模板和应用。
...
- `access` 被 `template` 和 `application` 调用以校验资源归属、ADMIN 身份和 SUPER_ADMIN 身份。
...
- 管理员可以读取和维护全部用户的模板、应用和字段映射。
- 超级管理员可以读取和维护全部用户的模板、应用和字段映射。
