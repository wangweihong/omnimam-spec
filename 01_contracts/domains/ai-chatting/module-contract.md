...
- 不创建独立模型配置表或同等模型事实源。
- 不提供模型创建、同步、检测、默认模型保存等能力；这些能力属于 `model-preferences`。
...
| 模块 | 输入 | 输出 |
| --- | --- | --- |
| topic | 当前用户、assistantId、providerResourceId、标题、分支来源 | Topic |
...
| assistant | 助手配置、suggestedProviderResourceId | Assistant |
...
- 依赖 `model-preferences` 读取 `assistant.default`、`translation` 默认 ProviderResource ID 与展示偏好；Model Preferences 不提供健康、能力或执行资格事实。
- 依赖 `modelgateway` 通过 `ResolveProviderTarget` 重新校验 ProviderAccount/ProviderResource 的作用域、能力、健康和修订，并使用不透明 `provider-execution-grant://` 执行标准 Operation。
- 被 Web `/ai-chatting` 页面依赖。

## 跨模块调用规则

- `providerResourceId` 和 `suggestedProviderResourceId` 必须引用 `modelgateway.ProviderResource.id`；Model Preferences 只保存该引用及展示覆盖。
- `modelSnapshot` 只记录生成或翻译当时的模型事实快照，不代表 ai-chatting 拥有模型配置。
- Topic 和 Assistant 的模型引用通过 Model Preferences 当前用户受控批量只读能力返回一跳摘要；Topic 与助手级 QuickPhrase 的 Assistant 摘要由 ai-chatting 同域批量读取。关联缺失或不可见时保留原 ID、摘要为空，列表不得逐行补查。
...
- 每次执行先调用 `ResolveProviderTarget(principal, target_selection, capability_definition_id, execution_ref)`；跨用户、停用、非 healthy、能力不匹配或修订失效时不得创建 Provider 调用。
- 解析成功后以不透明 Grant 调用 Gateway `ExecuteOperation`。AI Chat 不提交客户端构造的 Provider 地址、凭证、Adapter ID、Executor ID 或 Grant 解析结果。
- GenerationRun 必须保存 `provider_resource_id`、`account_scope`、`provider_account_config_version`、可选 `provider_resource_revision`、`provider_capability_revision` 和非敏感 `model_snapshot_json`；Gateway 返回只推进 AI Chat 自有状态。
