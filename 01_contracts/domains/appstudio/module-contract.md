...
当前 AppStudio S1 使用 `US-APPSTUDIO-001`、`BR-APPSTUDIO-001`、`AC-APPSTUDIO-001-01..18` 及 `R-STUDIO-*` 规则。OpenAPI、Schema、错误、权限和事件必须同时遵守 Workspace 后端内化、GitLab-only 正文、Blueprint 固定版本、Coding Agent generation、消息/事件 facade、Runtime 脱敏诊断、canonical 源码谱系、Artifact 成功门禁和健康切换/回滚语义。
