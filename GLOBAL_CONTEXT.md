...
| `model-deployment` | 管理平台共享 vLLM/LM Studio 模型部署、不可变 Spec Revision、Rollout 和生命周期投影；不拥有 Model Gateway 调用事实。 |
...
- `CreateStudioApplication` 不接受 Workspace、Blueprint 或 GitLab Server 输入；当前 `STATIC_WEB` 固定使用内置 `web-react@v1`。后端先返回持久化的 CREATING Application 与初始化 DAG ID，再通过唯一 READY 默认 GitLabServer 幂等创建 Project、Starter Template/Hook、Repository、唯一默认编辑上下文、Coding Agent、Session 和首次 Invocation。用户仍只通过 StudioApplication 级 Source/Revision、Snapshot 和 Preview 接口操作源码。
...
- Task Center 使用版本化只读 Function Registry 校验 Agent/AppStudio 与 Model Deployment 的 14 个 canonical functionRef，并在 AtomicTask 创建时固定合同 version/digest；调用方不能覆盖执行模式、能力或 Infra 映射，registry 升级不能改写历史任务。
- `gitlab.pipeline.run` 仍是独立 GitLab 领域的非 Infra-backed 外部 AtomicTask，使用 external_job_id 和延迟回调恢复，不进入 Agent/AppStudio Infra Function Registry。受信 AppStudio Push DAG可以调用它并在 Artifact 完成后从固定 Revision 启动 Preview；公共 DAG API 不能选择该 handler，Production 仍由 StudioRelease 状态机拥有。
...
- `model-deployment` 通过 Task Center 独立 `model-deployment.<engine>.*@2.0` DAG 固定 Rollout/Revision/digest；Worker resolver 读取完整配置，Infrastructure 按 `runtime_provider=docker` 物化 STRUCTURED/NATIVE Runtime。新 Runtime 通过 RUNNING/Endpoint READY/HEALTHY 门禁后才切换 active Revision。
- Model Deployment、Task Center 与 Infrastructure 的高级自定义合同由 `spec-v1.25.3` 发布并彻底替换 `spec-v1.24.x`；实施前必须按 gate 定向清理旧 Runtime、Task/DAG、部署数据和事件投影。
