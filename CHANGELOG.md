...
- `appstudio.build.execute@1.1` 成为 ACTIVE，固定 `bundle.tar.gz` 输出和 `infra.output.collect`、`infra.output.content.read` 能力；Task Worker 按 Infra 流读取、Asset Library `create -> upload -> complete`、大小/digest 双重校验和 attach 流程交付 Artifact。`1.0` 保留为 RETAINED 且历史 digest 不改写，本轮仍为待新 Release 草稿，未修改 `RELEASE.md`。
